...
import { Sun, HardDrive } from "lucide-react";
  ...
  return (
    <Card className="h-full">
      <CardContent className="pt-6 space-y-4">
        <StatRow icon={Sun} label="CPU" value={`${stats.CPUTemp.toFixed(0)}°C`} iconClass="text-warning" />
        <StatRow icon={HardDrive} label="NVMe" value={`${stats.NVMeTemp.toFixed(0)}°C`} iconClass="text-primary" />
      </CardContent>
    </Card>
  );
}
